...
### Error: `Failed to execute 'json' on 'Response': Unexpected token 'A'...`
This means the serverless function crashed and returned an error page instead of JSON. The most common causes:

1. **Missing environment variables** — Make sure all 3 variables (`SUPABASE_URL`, `SUPABASE_ANON_KEY`, `AI_API_KEY`) are added in **Vercel → Project → Settings → Environment Variables**, then **redeploy** (Vercel → Deployments → ⋯ → Redeploy). Environment variables only apply to new deployments!
2. **Old code** — Make sure you pushed the latest code (the API uses `pdf-parse` v2's `PDFParse` class and a native Vercel function). Re-upload to GitHub if unsure.
3. **Check the logs** — In Vercel, go to your project → **Logs** (or open the failed function in **Deployments**) to see the exact error message.
